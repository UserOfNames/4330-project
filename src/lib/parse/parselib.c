...
int assign(Variable **table) {
    Token value;
    // I have no idea what to name this variable (p2 is 'plus 2'), but it's
    ...
    set_variable(table, _IP -> literal.Name, value);
}


int parse_identifier(Variable **table) {
    // Check the type of the next token for what to do
    switch ((_IP) -> type) {
        // Assignment
        case EQ:
            return assign(table);
